...
pub fn write_nonempty_frontier_v1<H: HashSer, W: Write>(
    ...
    writer.write_u64::<LittleEndian>(<usize>::from(frontier.position()) as u64)?;
    match frontier.leaf() {
        Leaf::Left(a) => {
            a.write(&mut writer)?;
...
pub fn write_auth_fragment_v1<H: HashSer, W: Write>(
    ...
    writer.write_u64::<LittleEndian>(<usize>::from(fragment.position()) as u64)?;
...
pub fn write_bridge_v1<H: HashSer, W: Write>(
    ...
    Optional::write(
        &mut writer,
        &bridge.prior_position().map(|p| <usize>::from(p) as u64),
