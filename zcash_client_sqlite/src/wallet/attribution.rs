...
/// The spend evidence by which a repair pass selects the transactions to revisit.
///
/// A pass can only read the spend tables that exist where it sits in the migration graph, and the
/// Ironwood tables are newer than the first two passes.
pub(crate) enum RepairScope {
    /// Transactions recorded as spending one of the wallet's transparent outputs.
    ///
    /// Only the two passes that predate the pool-complete selection use this, and both are
    /// gated on `transparent-inputs`, so a build without that feature constructs it nowhere.
    #[cfg_attr(not(feature = "transparent-inputs"), allow(dead_code))]
    TransparentSpends,
    /// Transactions recorded as spending anything the wallet received, in any pool.
    AnySpend,
}

impl RepairScope {
    /// The `SELECT` yielding the identifiers of the transactions this scope covers.
    fn spend_sources(&self) -> &'static str {
        match self {
            RepairScope::TransparentSpends => {
                "SELECT transaction_id FROM transparent_received_output_spends"
            }
            RepairScope::AnySpend => {
                "SELECT transaction_id FROM transparent_received_output_spends
...
/// of its own outputs, within the given scope.
///
...
/// already attributed.
///
/// `scope` names the spend evidence to select by. A wider scope costs nothing beyond the two
/// counts an already-attributed transaction settles for, so the choice is governed by which spend
/// tables exist where the caller sits in the migration graph, not by which transactions need
/// repair.
...
pub(crate) mod tests {
    ...
    use super::{RepairScope, repair_funding_attribution};
    ...
    fn shielded_spend_attribution_is_idempotent() {
        ...
        let repair = |st: &TestState<BlockCache, TestDb, LocalNetwork>| {
            let conn = st.wallet().db().conn.unchecked_transaction().unwrap();
            repair_funding_attribution(&conn, &network, RepairScope::AnySpend).unwrap();
            conn.commit().unwrap();
        };
    ...
    ///
    /// This is the state the two passes before this one could not reach, because they selected
    /// the transactions to repair by transparent spend evidence and this transaction has none.
    #[test]
    fn repair_heals_a_shielded_funded_transfer() {
        ...
        // The narrow selection the previous passes used reaches nothing here.
        {
            let conn = st.wallet().db().conn.unchecked_transaction().unwrap();
            repair_funding_attribution(&conn, &network, RepairScope::TransparentSpends).unwrap();
            conn.commit().unwrap();
        }
        assert_eq!(
            pool_output_parties(&st.wallet().db().conn, txid, sapling),
            ...
            "a transaction with no transparent spend is outside the earlier selection",
        );

        // This migration's selection does.
        let migration =
            crate::wallet::init::migrations::repair_shielded_spend_attribution_migration(network);
        let repair = |st: &TestState<BlockCache, TestDb, LocalNetwork>| {
            let conn = st.wallet().db().conn.unchecked_transaction().unwrap();
