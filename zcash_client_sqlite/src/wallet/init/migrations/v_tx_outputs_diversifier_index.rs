//! Exposes the receiving address's diversifier index in `v_tx_outputs`.
//!
//! The `unioned` common table expression of `v_tx_outputs` selects
//! `addresses.diversifier_index_be` for each output the wallet received, but the aggregation
//! that merges the received-output and sent-output rows of an output never carried the column
//! into the view's result. A consumer could therefore learn which address received an output,
//! but not the diversifier index at which that address was derived, without joining back to
//! the `addresses` table.
//!
//! This migration recreates `v_tx_outputs` with the column selected, alongside the other
//! per-output values the aggregation retains.
...
/// This migration exposes the diversifier index of the receiving address in `v_tx_outputs`.
...
impl schemerz::Migration<Uuid> for Migration {
    ...
    fn description(&self) -> &'static str {
        "Exposes the diversifier index of the receiving address in v_tx_outputs."
    }
}
